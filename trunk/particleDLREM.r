...
pDLR.filter <- function(y,x,mean,cov,beta,npart,resample=NULL) {
    # state: npart * nvar * T array
    # weight: npart * T array
    x <- as.matrix(x)
    nt <- length(y)
    nx <- ncol(x)
    xnames <- dimnames(x)[[2]]
    state <- array(dim=c(npart,nx,nt+1))
    weight <- array(1/npart,dim=c(npart,nt+1))
    start.state[,,1] <- z <- mvrnorm(n = npart, mu=mean, Sigma=cov)
    for(i in 1:length(y)) {
        z <- mvrnorm(n=npart,mu=rep(0,ncol(z)),Sigma=cov) + z
        w <- logis(z%*%x[i,]) # likelihood of y=1
        w <- y[i]*w + (1-y[i])*(1-w) # importance weights
        weight[,i+1] <- w <- w/sum(w) # normalize importance weights
        #weight[i,] <- w%*%z # compute mean weights before resampling
        if(!is.null(resample) && i %% resample == 0) {
            rs <- sample(1:npart,size=npart,replace=TRUE,prob=w) # selection step
            state[,,i+1] <- z <- z[rs,]
            weight[,i+1] <- 1/npart # uniform weights
        } else {
            state[,,i+1] <- z
        }
...
pDLR.smoother <- function(state,cov,npart,weight=NULL) {
    np <- dim(state)[1]
    nx <- dim(state)[2]
    nt <- dim(state)[3]
    smooth <- array(dim=c(npart,nx,nt))
    mean <- rep(0,nx)
    if(!is.null(weight)) {
        rs <- sample(1:npart,size=npart,replace=TRUE,prob=weight[,nt]) # selection step
        for(t in (nt-1):1) {
        #apply(smooth[,,t+1],1,"-",state[,,t])
            p <- dmvnorm(smooth[,,t+1]-state[,,t],mean=mean,sigma=cov)
            rs <- sample(1:npart,size=npart,replace=TRUE,prob=weight[,t]) # selection step
        }
    } else {
        rs <- sample(1:npart,size=npart,replace=TRUE) # selection step
        for(t in (nt-1):1) {
            apply(smooth[,,t+1],1,"-",state[,,t])
            p <- dmvnorm(smooth[,,t+1]-state[,,t],mean=mean,sigma=cov)
            rs <- sample(1:npart,size=npart,replace=TRUE,prob=weight[,t]) # selection step
    ...
    smooth <- state[rs,,nt]
    #state[mvrnorm(n=npart,mu=rep(0,ncol(z)),Sigma=cov) + z
    #for(i in 1:npart) {
    for(t in (nt-1):1) {
        #apply(smooth[,,t+1],1,"-",state[,,t])
        p <- dmvnorm(smooth[,,t+1]-state[,,t],mean=mean,sigma=cov)
