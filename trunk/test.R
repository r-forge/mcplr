...
dat <- subset(WPT,id=="C")
lmod <- RescorlaWagner(y~x1+x2+x3+x4,data=dat,parameters=list(alpha=.1,beta=c(1,1),lambda=c(1,-1)),remove.intercept=TRUE,base=1,fix=list(beta=FALSE,ws=TRUE,lambda=TRUE))
